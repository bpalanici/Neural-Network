...
#train : first 200 - 250 epochs with image generator
# next 200 - 250 epochs without image generator
...
#model = define_model()
model = load_model("model_accuracy_247.h5")
print(model.summary())
#exit(2)
    ...
    def on_epoch_end(self, epoch, logs=None, max_accuracy=max_accuracy, min_loss=min_loss):
        if epoch > 20:
            if logs["val_loss"] < min_loss.value:
                ...
                self.model.save("model_loss_{}.h5".format(epoch + 247))
            if logs["val_accuracy"] > max_accuracy.value:
                ...
                self.model.save("model_accuracy_{}.h5".format(epoch + 247))
