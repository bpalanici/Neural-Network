...
def define_model():
    ...
    modelIn = Embedding(nr_cuv_diferite, 128, input_length=dim_max)(input)

    Gru1 = GRU(units=256, return_sequences=True,
               activation=activation, kernel_initializer=kernel)(modelIn)

    # Second GRU layer
    Gru2 = GRU(units=256, return_sequences=True,
               activation=activation, kernel_initializer=kernel)(modelIn)

    Gru3 = GRU(units=256, return_sequences=True,
               activation=activation, kernel_initializer=kernel)(modelIn)
    ...
    # third GRU layer
    Gru4 = GRU(units=128, return_sequences=True,
               activation=activation, kernel_initializer=kernel)(model)

    # third GRU layer
    Gru5 = GRU(units=128, return_sequences=True,
               activation=activation, kernel_initializer=kernel)(model)
    # add layer
    model = Add()([Gru4, Gru5])
    model = Dropout(0.2)(model)

    Gru6 = GRU(units=128, return_sequences=True,
               activation=activation, kernel_initializer=kernel)(model)
